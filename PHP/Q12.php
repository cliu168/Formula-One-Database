<head> 
  <title>Driver Wins at Specific Circuit</title>
 </head>
 <body>
...
if (!$db) {
  ...
  $result = mysqli_query($db, "CALL Q12('".$input."')");
  // call to procedure
  ...
  if (!$result || $result->num_rows == 0) {
    ...
    echo "<table border=1>\n";
    echo "<tr><td>FirstName</td><td>LastName</td><td>Wins</td></tr>\n";
    $not_done = 0;
    while ($myrow = mysqli_fetch_array($result)) {
        printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>\n", 
        $myrow["fname"], $myrow["lname"], $myrow["numWins"]);
        if ($not_done < 10) {
            $not_broken = iconv("UTF-8", "UTF-8//IGNORE", $myrow["lname"]);
            array_push($dataPoints, array("label"=> $not_broken, "y"=> $myrow["numWins"])); 
    ...
    <script>
      ...
      window.onload = function () {
        var chart = new CanvasJS.Chart("chartContainer", {
          ...
          theme: "light1",
          title: {
            text: "Driver Wins at Specific Circuit"
          },
          data: [{
            type: "column",
