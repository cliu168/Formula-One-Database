<head> 
  <title>Average number of points per season for driver birthyears</title>
 </head>
 <body>
...
if (!$db) {
  ...
  $result = mysqli_query($db, "CALL Q20()");
  // call to procedure
  ...
  if (!$result) {
    ...
    echo "<table border=1>\n";
    echo "<tr><td>BirthYear</td><td>AveragePts</td></tr>\n";
    $not_done = 0;
    while ($myrow = mysqli_fetch_array($result)) {
        printf("<tr><td>%s</td><td>%.2f</td></tr>\n", 
        $myrow["bYear"], $myrow["avgPts"]);
        if ($not_done < 10) {
            $not_broken = iconv("UTF-8", "UTF-8//IGNORE", $myrow["bYear"]);
    ...
    <script>
      ...
      window.onload = function () {
        var chart = new CanvasJS.Chart("chartContainer", {
          ...
          theme: "light1",
          title: {
            text: "Average number of points per season for driver birthyears"
          },
          data: [{
            type: "column",
