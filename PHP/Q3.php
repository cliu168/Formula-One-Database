<head>
  <title>Number of Circuits by Country</title>
 </head>
 <body>
...
if (!$db) {
  ...
  $result = mysqli_query($db, "CALL Q3()");
  // call to procedure
  ...
  if (!$result || $result->num_rows == 0) {
    ...
    while ($myrow = mysqli_fetch_array($result)) {
      printf("<tr><td>%s</td><td>%s</td></tr>\n", 
        ...
        if ($not_done < 10) {
        array_push($dataPoints, array("label"=> $myrow["country"], "y"=> $myrow["numCircuits"]));
        }
      $not_done++;
    }
    echo "</table>\n";
    ...
    <script>
      ...
      window.onload = function () {
        var chart = new CanvasJS.Chart("chartContainer", {
          ...
          title: {
            text: "Number of Circuits by Country"
          },
          data: [{
            type: "column",
