...
BEGIN
    SELECT nationality, COUNT(driverID) as numDrivers
    FROM Drivers
...
CREATE OR REPLACE PROCEDURE Q2(IN mynationality VARCHAR(100))
BEGIN
    SELECT fName, lName
    FROM Drivers
    WHERE nationality = mynationality
    ORDER BY lName;
END; //
...
CREATE OR REPLACE PROCEDURE Q4(IN mycountry VARCHAR(100))
BEGIN
...
BEGIN
    ...
    WHERE D.driverID = RR.driverID AND RR.finalPosition = 1
...
BEGIN
    ...
    WHERE D.driverID = S.driverID AND S.finalposition = 1
    ORDER BY D.lname ASC;
END; //
...
BEGIN
    SELECT D.fname, D.lname, AVG(R.seasonPts) As avgPts
    FROM (
...
BEGIN
    SELECT CS.name, AVG(R.seasonPts) AS avgPts
    FROM (
...
BEGIN
    SELECT D.nationality, AVG(R.seasonPts) AS avgPts
    FROM (
        SELECT driverID, year, SUM(points) AS seasonPts
        FROM RacePointsView
        GROUP BY driverID, year
...
BEGIN
    SELECT YEAR(D.dob) AS bYear, AVG(R.seasonPts) AS avgPts
    FROM (
        SELECT driverID, year, SUM(points) AS seasonPts
        FROM RacePointsView
        GROUP BY driverID, year
